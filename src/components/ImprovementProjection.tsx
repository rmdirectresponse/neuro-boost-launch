...
const ImprovementProjection = () => {
  ...
  const progressData = [
    { days: 7, label: "Protocol Start", progress: 25, color: "bg-yellow-500" },
    { days: 14, label: "Continuing", progress: 60, color: "bg-orange-500" },
    { days: 21, label: "Continuing", progress: 90, color: "bg-secondary" }
  ];
    ...
    <section className={`py-12 px-4 transition-opacity duration-800 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="max-w-6xl mx-auto">
        <h3 className="neuro-headline text-center mb-4">
          After the NeuroActive material:
        </h3>
        
        <p className="text-center neuro-body text-muted-foreground mb-12">
          Improvement projection with the mental exercise program
        </p>
          ...
          {progressData.map((item, index) => (
            <div key={index} className="neuro-card text-center">
              <div className="mb-6">
                ...
                <h4 className="text-xl font-bold mb-2">{item.days} days</h4>
